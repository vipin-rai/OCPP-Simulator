...
export class MeterValueScheduler {
  private baseMeterValue: number | null = null;
   private lastCurveValueWh: number = 0;
  ...
  start(strategy: MeterValueStrategy): void {
    ...
    this.strategy = strategy;
      console.log("🚨 MeterValueScheduler.start() CALLED FROM WHERE???");
  console.log("Stack trace:", new Error().stack);

    if (strategy.kind === "curve") {
       this.baseMeterValue = this.callbacks.getCurrentValue(); // 🔥 IMPORTANT
    this.lastCurveValueWh = 0;   // 🔥 reset
  ...
  // }
    ...
    private tickCurve(config: AutoMeterValueConfig): void {
  if (!this.startTimestamp || this.baseMeterValue === null) return;

  const elapsedMs = Date.now() - this.startTimestamp;
  const elapsedMinutes = elapsedMs / 1000 / 60;

  const curveValueKWh = getMeterValueAtTime(elapsedMinutes, config);
  const curveValueWh = Math.round(curveValueKWh * 1000);

  // 🔥 MAIN FIX: base + curve
  const nextMeterValue = this.baseMeterValue + curveValueWh;

  this.callbacks.updateValue(nextMeterValue);
  this.callbacks.onSend(this.connectorId);
}
